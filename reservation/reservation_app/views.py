...
class AddRoom(View):
    ...
    def post(self, request):
        ...
        if not name:
            return render(
                request, "add_room.html", context={"error": "Room name not entered"}
            )
        if capacity <= 0:
            return render(
                request,
    ...
    def post(self, request, room_id):
        ...
        if not name:
            return render(
                request,
                "modify_room.html",
                context={"room": room, "error": "Room name not entered"},
            )
        if capacity <= 0:
            return render(
                request,
